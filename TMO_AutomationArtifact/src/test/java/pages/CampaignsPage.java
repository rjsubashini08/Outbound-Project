...
//	private By campaignsContext=By.xpath("//div[@class='radioTable']/div/span/label[contains(text(),'Customer')]");
	...
	private By datAudienceNumb=By.xpath("(//span[@class='leftJustifyStyle'])[1]");
	private By datAudienceCust=By.xpath("//*[@id='CT']/div/div/div/div[1]/div/div/div/div[2]/div/div/div/div[2]");
	private By resultStrategy=By.xpath("//span[contains(text(),'Strategy')]");
	private By dataStrategyNum=By.xpath("(//span[@class='rightJustifyStyle'])[1]");
	private By dataStrategyOfferRes=By.xpath("//span[contains(text(),'offer results')]");
	private By resultInitiated=By.xpath("//span[contains(text(),'Initiated')]");
	private By dataInitiatedNum=By.xpath("(//span[@class='rightJustifyStyle'])[2]");
	private By dataInitiatedOffIni=By.xpath("//span[contains(text(),'offers initiated')]");
	private By result=By.xpath("//span[contains(text(),'Result')]");
	private By dataResultNum=By.xpath("(//span[@class='rightJustifyStyle'])[3]");
	private By dataResultComp=By.xpath("//span[contains(text(),'completed')]");
	private By dataResultOffersSel=By.xpath("//span[contains(text(),'offers selected')]");
	private By dataResult=By.xpath("//*[@id='CT']/div/div/div/div[4]/div/div/div/div[2]/div/div/div/div[1]");
	...
	private By refreshBtn=By.xpath("//i[@class='pi pi-refresh']");
	private By wrapUpStatus=By.xpath("//span[contains(text(),'Wrap up')]");
	private By runStatus=By.xpath("//span[contains(text(),'Running')]");
	private By statusBtn=By.xpath("//button[@name='MKTCaseHeader_pyWorkPage_17']");
	private By ValStatus=By.xpath("//span[@class='badge_text'][@data-test-id='20150625144845006013966']");
...
// System.out.println("Total Numbe rof Frames "+noiframe);
		...
		driver.findElement(campaignsName).sendKeys("AUTOCamp"+b);
		Thread.sleep(2000);
		...
		driver.findElement(campaignsProgName).click();
		Thread.sleep(1000);
		driver.findElement(campaignsProgName).sendKeys("AUTOPr"+b);
		Thread.sleep(2000);
...
//		Thread.sleep(3000);
	...
	public void validateResult() throws InterruptedException {
		...
		System.out.println("After clicking on Refresh Button");
		Thread.sleep(7000);
		driver.findElement(refreshBtn).click();
		Thread.sleep(5000);
		WebElement wrapUpStatusEle=driver.findElement(ValStatus);
		String valWrapUpStatus=wrapUpStatusEle.getText();
		while(!valWrapUpStatus.equals("WRAP UP"))
	{ 	
			WebElement wrapUpStatusEle1=driver.findElement(ValStatus);
			String valWrapUpStatus1=wrapUpStatusEle1.getText();
			if(valWrapUpStatus1.equals("RUNNING"))
			{
			System.out.println("Now the Status is "+valWrapUpStatus1);
			driver.findElement(refreshBtn).click();
			Thread.sleep(15000);
			System.out.println("The status Still in "+valWrapUpStatus1  + "Status");
			}
			else
			{
				System.out.println("Now the Status is "+valWrapUpStatus1);
		...
		Boolean audBoolean=driver.findElement(resultAudience).isDisplayed();
		System.out.println("Audience Header is displayed in Result page "+audBoolean);
		String audDataCust=driver.findElement(datAudienceCust).getText();
		String audDataNum=driver.findElement(datAudienceNumb).getText();
		System.out.println("Audience Data "+audDataCust);
		System.out.println("Audience Data "+audDataNum);
		Assert.assertEquals(audDataNum,"5" );
		Assert.assertEquals(audDataCust,"customers" );
		
		//Strategy Validations
		Boolean stratBoolean=driver.findElement(resultStrategy).isDisplayed();
		System.out.println("Strategy Header is displayed in Result page "+stratBoolean);
		String strategyDataNum=driver.findElement(dataStrategyNum).getText();
		String strategyDataOfferres=driver.findElement(dataStrategyOfferRes).getText();
		System.out.println("Strategy Data Numbers "+strategyDataNum);
		System.out.println("Strategy Data Offers "+strategyDataOfferres);
		Assert.assertEquals(strategyDataNum,"5" );
		Assert.assertEquals(strategyDataOfferres,"offer results" );

		
		//Initiated Validations
		Boolean InitiatedBoolean=driver.findElement(resultInitiated).isDisplayed();
		System.out.println("Initiated Haeder is displayed in Result page "+InitiatedBoolean);
		String InitiatedDataNum=driver.findElement(dataInitiatedNum).getText();
		String InitiatedDataOffIni=driver.findElement(dataInitiatedOffIni).getText();
		System.out.println("Initiated Data Number "+InitiatedDataNum);
		System.out.println("Initiated Data Offer Initiated "+InitiatedDataOffIni);
		Assert.assertEquals(InitiatedDataNum,"5" );
		Assert.assertEquals(InitiatedDataOffIni,"offers initiated" );
		
		//Results Validations
		Boolean resultBoolean=driver.findElement(result).isDisplayed();
		System.out.println("Results Header is displayed in Result page "+resultBoolean);
		String resultDataNum=driver.findElement(dataResultNum).getText();
		String resultDataComp=driver.findElement(dataResultComp).getText();
		String resultDataOffSel=driver.findElement(dataResultOffersSel).getText();
		System.out.println("Results Data Number"+resultDataNum);
		System.out.println("Results Data Completed"+resultDataComp);
		System.out.println("Results Data Completed"+resultDataOffSel);
		Assert.assertEquals(resultDataNum,"5" );
		Assert.assertEquals(resultDataComp,"completed" );
		Assert.assertEquals(resultDataOffSel,"offers selected" );
